...
foreach my $q (@class_query) { @list = grep { /^(?:[^<]*<>){15}[^<]*?\Q$q\E/ } @list; }
...
elsif($sort eq 'lv')   { my @t = map { (/^(?:[^<]*<>){14}([^<]*)/)[0] } @list; @list = @list[sort {$t[$b] <=> $t[$a]} 0 .. $#t]; }
elsif($sort eq 'exp')  { my @t = map { (/^(?:[^<]*<>){13}([^<]*)/)[0] } @list; @list = @list[sort {$t[$b] <=> $t[$a]} 0 .. $#t]; }
