...
if($tag_query) { @list = grep { /^(?:[^<]*<>){18}[^<]*? \Q$tag_query\E / } @list; }
...
elsif($sort eq 'level'){ my @t = map { (/^(?:[^<]*<>){15}([^<]*)/)[0] } @list; @list = @list[sort {$t[$b] <=> $t[$a]} 0 .. $#t]; }
