...
if(!($mode eq 'mylist' || $::in{tag} || $::in{group} || $::in{name} || $::in{player} || $::in{race} || $::in{class} || $::in{style} || $::in{image})){
  $index_mode = 1;
...
if($race_query) { @list = grep { /^(?:[^<]*<>){10}\Q$race_query\E/ } @list; }
$INDEX->param(race => $race_query);

## クラス検索
my $class_query = decode('utf8', $::in{class});
if($class_query) { @list = grep { /^(?:[^<]*<>){11}\Q$class_query\E/ } @list; }
$INDEX->param(class => $class_query);

## スタイル検索
my @style_query = split('\s', decode('utf8', $::in{style}));
foreach my $q (@style_query) { @list = grep { /^(?:[^<]*<>){12}[^<]*?\Q$q\E/ } @list; }
...
elsif($sort eq 'age')   { my @t = map { (/^(?:[^<]*<>){16}([^<]*)/)[0] } @list; @list = @list[sort {$t[$a] cmp $t[$b]} 0 .. $#t]; }
...
foreach (@list) {
  my (
    ...
    $image, $tags, $hide, #7-9
    $race, $class, $styles, #10-12
    $level, $point, #13-14
    $gender, $age, $height, #15-17
    $session,
  ) = (split /<>/, $_)[0..18];
