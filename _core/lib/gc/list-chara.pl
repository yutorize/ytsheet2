...
if($class_query) { @list = grep { /^(?:[^<]*<>){10}\Q$class_query\E/ } @list; }
$INDEX->param(class => $class_query);

## スタイル検索
my $style_query = decode('utf8', $::in{style});
if($style_query) { @list = grep { /^(?:[^<]*<>){11}\Q$style_query\E/ } @list; }
$INDEX->param(style => "$style_query");

## ワークス検索
my $works_query = decode('utf8', $::in{works});
if($works_query) { @list = grep { /^(?:[^<]*<>){13}\Q$works_query\E/ } @list; }
...
elsif($sort eq 'lv')   { my @t = map { (/^(?:[^<]*<>){13}([^<]*)/)[0] } @list; @list = @list[sort {$t[$b] <=> $t[$a]} 0 .. $#t]; }
...
foreach (@list) {
  my (
    ...
    $image, $tags, $hide, #7-9
    $class, $style, $substyle, $works, #10-13
    $level, $exp, #14-15
    $country, $gender, $age, $height, $weight, #16-20
    $session,
  ) = (split /<>/, $_)[0..21];
  ...
  ## 通常リスト
    ...
    if(length $style    >= 8){ $style   = "<span class='thin'>$style</span>" }
    if(length $substyle >= 8){ $substyle= "<span class='thin'>$substyle</span>" }
    if(length $works    >= 8){ $works   = "<span class='thin'>$works</span>" }
