...
if($style_query) { @list = grep { $_ =~ /^(?:[^<]*?<>){11}\Q$style_query\E/ } @list; }
$INDEX->param(style => "$style_query");

## ワークス検索
my $works_query = decode('utf8', $::in{works});
if($works_query) { @list = grep { $_ =~ /^(?:[^<]*?<>){13}\Q$works_query\E/ } @list; }
...
elsif($sort eq 'lv')    { my @tmp = map { (split /<>/)[13]          } @list; @list = @list[sort {$tmp[$b] <=> $tmp[$a]} 0 .. $#tmp]; }
