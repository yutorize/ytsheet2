...
if(!($mode eq 'mylist' || $::in{tag} || $::in{group} || $::in{name} || $::in{player} || $::in{lvmin} || $::in{lvmax} || $::in{rule} || $::in{base} || $::in{belong} || $::in{leader} || $::in{image})){
  $index_mode = 1;
...
my %groups = groupArrayToHash(\@set::groups_clan);
$groups{all}{name} = 'すべて' if $::in{group} eq 'all' || !@set::groups_clan;
