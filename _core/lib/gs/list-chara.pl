...
if(!($mode eq 'mylist' || $::in{tag} || $::in{group} || $::in{name} || $::in{player} || $::in{race} || $::in{'exp-min'} || $::in{'exp-max'} || $::in{class} || $::in{faith} || $::in{image})){
  $index_mode = 1;
...
if($race_query) { @list = grep { /^(?:[^<]*<>){14}\Q$race_query\E/ } @list; }
$INDEX->param(race => $race_query);

## 経験点検索
my $exp_min_query = $::in{'exp-min'};
my $exp_max_query = $::in{'exp-max'};
if($exp_min_query) { @list = grep { (/^(?:[^<]*<>){11}([^<]*)/)[0] >= $exp_min_query } @list; }
if($exp_max_query) { @list = grep { (/^(?:[^<]*<>){11}([^<]*)/)[0] <= $exp_max_query } @list; }
...
if(@class_query){
  ...
  foreach my $class (@class_query){
    ...
    if($lv ne ''){
      if   ($op eq '>='){ @list = grep { (split '/',(/^(?:[^<]*<>){13}([^<]*)/)[0])[$num{$name}] >= $lv } @list; }
      elsif($op eq '<='){ @list = grep { (split '/',(/^(?:[^<]*<>){13}([^<]*)/)[0])[$num{$name}] <= $lv } @list; }
      elsif($op eq '>' ){ @list = grep { (split '/',(/^(?:[^<]*<>){13}([^<]*)/)[0])[$num{$name}] >  $lv } @list; }
      elsif($op eq '<' ){ @list = grep { (split '/',(/^(?:[^<]*<>){13}([^<]*)/)[0])[$num{$name}] <  $lv } @list; }
      else              { @list = grep { (split '/',(/^(?:[^<]*<>){13}([^<]*)/)[0])[$num{$name}] == $lv } @list; }
    }
    else { @list = grep { (split '/',(/^(?:[^<]*<>){13}([^<]*)/)[0])[$num{$name}] >= 1 } @list; }
  }
}
...
if($faith_query) { @list = grep { /^(?:[^<]*<>){19}[^<]*?\Q$faith_query\E/ } @list; }
...
foreach (0 .. $#set::adventurer_rank){
  $sortRank{$set::adventurer_rank[$_][0]} = $_;
  push(@rank_list, {
...
if($rank_query eq 'none') {
  ...
  @list = grep { /^(?:[^<]*<>){18}\Q$rank_query\E</ } @list;
...
elsif($sort eq 'exp')  { my @t = map { (/^(?:[^<]*<>){11}([^<]*)/)[0] } @list; @list = @list[sort {$t[$b] <=> $t[$a]} 0 .. $#t]; }
elsif($sort eq 'age')  { my @t = map { (/^(?:[^<]*<>){17}([^<]*)/)[0] } @list; @list = @list[sort {$t[$a] cmp $t[$b]} 0 .. $#t]; }

sub sortName { $_[0] =~ /^(?:[^<]*<>){4}(?:“\s*(.*?)”)?\s*(.*?)</; return $2 || $1; }
sub sortRank { return $sortRank{($_[0] =~ /^(?:[^<]*<>){18}([^<]*)/)[0]}; }
...
foreach (@list) {
  my (
    ...
    $exp, $level, $classes, #11-13
    $race, $racebase, $gender, $age, $rank, $faith, #14-19
  ) = (split /<>/, $_)[0..18];
  ...
  ## 通常リスト
    ...
    #種族
    $race =~ s/[（()].*[)）]|［.*］//;
    $race = "<span class=\"small\">$race</span>" if length($race) >= 8;
