...
if($race_query) { @list = grep { $_ =~ /^(?:[^<]*?<>){14}\Q$race_query\E/ } @list; }
$INDEX->param(race => $race_query);

## 経験点検索
my $exp_min_query = $::in{'exp-min'};
my $exp_max_query = $::in{'exp-max'};
if($exp_min_query) { @list = grep { (split(/<>/))[11] >= $exp_min_query } @list; }
if($exp_max_query) { @list = grep { (split(/<>/))[11] <= $exp_max_query } @list; }
...
if($faith_query) { @list = grep { $_ =~ /^(?:[^<]*?<>){19}[^<]*?\Q$faith_query\E/ } @list; }
...
if($rank_query eq 'none') {
  ...
  @list = grep { $_ =~ /^(?:[^<]*?<>){18}\Q$rank_query\E</ } @list;
...
elsif($sort eq 'rank'){ my @tmp = map { sortRank((split /<>/)[18])} @list; @list = @list[sort {$tmp[$b] <=> $tmp[$a]} 0 .. $#tmp]; }
elsif($sort eq 'lv')  { my @tmp = map { (split /<>/)[12]          } @list; @list = @list[sort {$tmp[$b] <=> $tmp[$a]} 0 .. $#tmp]; }
elsif($sort eq 'exp') { my @tmp = map { (split /<>/)[11]          } @list; @list = @list[sort {$tmp[$b] <=> $tmp[$a]} 0 .. $#tmp]; }
elsif($sort eq 'age') { my @tmp = map { (split /<>/)[17]          } @list; @list = @list[sort {$tmp[$a] cmp $tmp[$b]} 0 .. $#tmp]; }
