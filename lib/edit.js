...
function changeRace(){
  ...
  document.getElementById("language-default").innerHTML = raceLanguage[race] ? raceLanguage[race] : '<dt>初期習得言語</dt><dd>○</dd><dd>○</dd>';
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    if(level >= Number( array[i].replace(/[^0-9]/g, '') )){
      ...
      if (feat.match(/(ガーディアン)/)){
        ...
        box.value = RegExp.$1 + ( (f2 && (lvFig >= 9 || lvFen >= 9)) ? "Ⅱ" : "Ⅰ" );
      }
...
function calcMagic() {
  ...
  document.getElementById("magic-power-fairytamer-value").innerHTML = lvFai + bonusInt + magicPowerAdd;
...
function calcHonor(){
  ...
  const historyNum = form.historyNum.value;
  for (let i = 0; i <= historyNum; i++){
    let honor = Number(eval(form['history'+i+'Honor'].value));
    if(isNaN(honor)){ honor = 0; }
    honorTotal += honor;
  }
  const honorItemsNum = form.honorItemsNum.value;
  for (let i = 1; i <= honorItemsNum; i++){
    let honor = Number(eval(form['honorItem'+i+'Pt'].value));
    if(isNaN(honor)){ honor = 0; }
    honorTotal -= honor;
  }
  document.getElementById("honor-value").innerHTML = honorTotal;
}
