...
function calcSubStt() {
  ...
  document.getElementById("vit-resist-total").innerHTML = vitResistBase + Number(form.vitResistAdd.value);
  document.getElementById("mnd-resist-total").innerHTML = mndResistBase + Number(form.mndResistAdd.value);
  
  const hpBase = level * 3 + sttVit;
  const mpBase = (lvSor + lvCon + lvPri + lvMag) * 3 + sttMnd;
  const hpAutoAdd = tenacity;
  const mpAutoAdd = capacity;
  document.getElementById("hp-base").innerHTML = hpBase;
  document.getElementById("mp-base").innerHTML = mpBase;
  document.getElementById("hp-auto-add").innerHTML = hpAutoAdd;
  document.getElementById("mp-auto-add").innerHTML = mpAutoAdd;
  document.getElementById("hp-total").innerHTML = hpBase + Number(form.hpAdd.value);
  document.getElementById("mp-total").innerHTML = mpBase + Number(form.mpAdd.value);
}

// 移動力計算 //
function calcMobility() {
  let mobility = sttAgi + Number(form.mobilityAdd.value);
  document.getElementById("mobility-limited").innerHTML = 3;
  document.getElementById("mobility-base").innerHTML = sttAgi;
...
function calcExp(){
  ...
  for (let i = 0; i <= historyNum; i++){
    expTotal += Number(form['history'+i+'Exp'].value);
  }
