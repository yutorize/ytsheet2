  ...
  for (let i = 0; i < array.length; i++) {
    ...
    if(level >= Number( array[i].replace(/[^0-9]/g, '') )){
      ...
      if (feat.match(/(ガーディアン)/)){
        ...
        box.value = RegExp.$1 + ( (f2 && lvBar >= 7) ? "Ⅱ" : "Ⅰ" );
      }
        ...
        box.value = RegExp.$1 + ( (f2 && (lvFig >= 9 || lvGra >= 9)) ? "Ⅱ" : "Ⅰ" );
      }
...
function checkCraft() {
  ...
  for (let i = 1; i <= 19; i++) {
    ...
    if(i <= lvBar + songAddition){ cL.remove("fail"); }
    else {
      cL.add("fail");
      if(form.failHidden.checked){ cL.add("hidden") } else { cL.remove("hidden"); };
    }
...
// 魔力計算 //
function calcMagic() {
  const add = Number(form.magicPowerAdd.value);
