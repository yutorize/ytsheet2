...
function calcLv(){
  ...
  levelCasters = [lvSor, lvCon, lvPri, lvFai, lvMag];
...
function checkFeats(){
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    if(level >= Number( array[i].replace(/[^0-9]/g, '') )){
      ...
      if (feat.match(/(ガーディアン)/)){
        box.value = RegExp.$1 + ( (f2 && level >= 9) ? "Ⅱ" : "Ⅰ" );
      }
      else if (feat.match(/(回避行動)/)){
        box.value = RegExp.$1 + ( (f2 && lvFen >= 9) ? "Ⅱ" : "Ⅰ" );
      }
      else if (feat.match(/(呪歌追加)/)){
        box.value = RegExp.$1 + ( (f3 && lvBar >= 13) ? "Ⅲ" : (f2 && lvBar >= 7) ? "Ⅱ" : "Ⅰ" );
      }
      else if (feat.match(/(賦術強化)/)){
        box.value = RegExp.$1 + ( (f2 && lvAlc >= 9) ? "Ⅱ" : "Ⅰ" );
      }
      else if (feat.match(/(スローイング)/)){
        box.value = RegExp.$1 + ( (f2 && level >= 5) ? "Ⅱ" : "Ⅰ" );
      }
      else if (feat.match(/(変幻自在)/)){
        box.value = RegExp.$1 + ( (f2 && (lvGra >= 13 || lvFen >= 13)) ? "Ⅱ" : "Ⅰ" );
      }
      else if (feat.match(/(命中強化)/)){
        box.value = RegExp.$1 + ( (f2 && level >= 13) ? "Ⅱ" : "Ⅰ" );
      }
      else if (feat.match(/(跳び蹴り)/)){
        box.value = RegExp.$1 + ( (f2 && lvGra >= 9) ? "Ⅱ" : "Ⅰ" );
      }
      else if (feat.match(/(投げ強化)/)){
        box.value = RegExp.$1 + ( (f2 && lvGra >= 9) ? "Ⅱ" : "Ⅰ" );
      }
      else if (feat.match(/(魔力強化)/)){
        box.value = RegExp.$1 + ( (f2 && levelCasters[1] >= 10) ? "Ⅱ" : "Ⅰ" );
      }
        ...
        box.value = RegExp.$1 + ( (f2 && level >= 7) ? "Ⅱ" : "Ⅰ" );
      }
      else if (feat.match(/(斬り返し)/)){
        box.value = RegExp.$1 + ( (f2 && (lvFig >= 7 || lvFen >= 7)) ? "Ⅱ" : "Ⅰ" );
      }
      else if (feat.match(/(クリティカルキャスト)/)){
        box.value = RegExp.$1 + ( (f2 && level >= 11) ? "Ⅱ" : "Ⅰ" );
      }
      else if (feat.match(/(牽制攻撃)/)){
        box.value = RegExp.$1 + ( (f3 && level >= 11) ? "Ⅲ" : (f2 && level >= 7) ? "Ⅱ" : "Ⅰ" );
      }
      else if (feat.match(/(全力攻撃)/)){
        box.value = RegExp.$1 + ( (f3 && lvFig >= 15) ? "Ⅲ" : (f2 && (lvFig >= 9 || lvGra >= 9)) ? "Ⅱ" : "Ⅰ" );
      }
      else if (feat.match(/(バイオレントキャスト)/)){
        box.value = RegExp.$1 + ( (f2 && level >= 13) ? "Ⅱ" : "Ⅰ" );
      }
      else if (feat.match(/(必殺攻撃)/)){
        box.value = RegExp.$1 + ( (f3 && lvFen >= 11) ? "Ⅲ" : (f2 && level >= 7) ? "Ⅱ" : "Ⅰ" );
      }
      else if (feat.match(/(鎧貫き)/)){
        box.value = RegExp.$1 + ( (f2 && lvGra >= 15) ? "Ⅲ" : (f2 && lvGra >= 9) ? "Ⅱ" : "Ⅰ" );
      }
      ...
      cL.add("fail");
      if(form.failHidden.checked){ cL.add("hidden") } else { cL.remove("hidden"); };
    }
  }
...
function calcMagic() {
  ...
  document.getElementById("magic-power-alchemist-value").innerHTML  = lvAlc + parseInt((sttInt + sttAddE) / 6) + addAlc;
}
...
function calcWeapon() {
  ...
  for (let i = 1; i <= weaponNum; i++){
    ...
    if(attackClass) {
      accBase = attackClass + parseInt((sttDex + sttAddA + ownDex) / 6) + accuracyEnhance;
      dmgBase = attackClass;
      ...
      else if(category === '投擲')           { dmgBase += bonusStr + masteryThrow; accBase += throwing ? 1 : 0; }
...
function calcDefense() {
  ...
  document.getElementById("evasion-eva").innerHTML = evaClassLv + bonusAgi;
  // 種族特徴
...
function calcHonor(){
  ...
  const free = adventurerRank[rank]["free"];
  const historyNum = form.historyNum.value;
  for (let i = 0; i <= historyNum; i++){
...
function addWeapons(){
  ...
  cell3.innerHTML  = '+<input type="number" name="weapon' + num + 'Acc" oninput="calcWeapon()">=<b id="weapons' + num + '-acc-total"></b>';
  cell4.innerHTML  = '<input type="text" name="weapon' + num + 'Rate">';
  cell5.innerHTML  = '<input type="text" name="weapon' + num + 'Crit">';
  cell6.innerHTML  = '+<input type="number" name="weapon' + num + 'Dmg" oninput="calcWeapon()">=<b id="weapons' + num + '-dmg-total"></b>';
...
function switchHistory(num){
  var Date  = form['history' + num + 'Date'  ].value;
  var Name  = form['history' + num + 'Title' ].value;
  var Exp   = form['history' + num + 'Exp'   ].value;
  var Money = form['history' + num + 'Honor' ].value;
  var Honor = form['history' + num + 'Money' ].value;
  var Grow  = form['history' + num + 'Grow'  ].value;
  var Gm    = form['history' + num + 'Gm'    ].value;
  var Member= form['history' + num + 'Member'].value;
  var Note  = form['history' + num + 'Note'  ].value;
